...
export default function CreatePlanet() {
  const {
    account, token, sendSolPayment, sendUsdcPayment, hasPremium,
  ...
  const handleGenerate = () => {
    ...
    if (!prompt.trim()) {
      alert('Please describe your planet first.')
      return
    }
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      // Show the generated image and allow naming
      setPreview({
  ...
  // ----- STEP 2 : Save with name & description -----
  ...
  const handleDiscard = () => {
    ...
    setName('')
    setDescription('')
    // Keep the prompt to allow another generation
  }
  ...
  return (
    <div className="max-w-2xl mx-auto text-center space-y-10">
      ...
      <div className="text-sm text-gray-400">
        {connected ? (
          <>Free generations left: <span className="text-purple-300">{freeGenerations}</span>
            {premium && <span className="ml-2 text-green-300">(Premium ♾️)</span>}</>
        ) : 'Connect wallet to see free generations.'}
      </div>

      {/* Prompt input – only shown when no preview */}
      {!preview && (
        <>
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            placeholder="Describe your planet (e.g., a golden desert planet with crystal rings)..."
            className="w-full h-32 bg-white/10 rounded-xl p-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
          <GlowButton onClick={handleGenerate} disabled={loading}>
            {loading ? 'Processing...' : 'Generate AI Planet'}
          </GlowButton>
        </>
      )}

      {/* Preview & naming section */}
      {preview && (
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
        ...
        >
          <img
            ...
            alt="Generated planet"
            className="mx-auto w-64 h-64 rounded-full shadow-[0_0_50px_rgba(168,85,247,0.5)]"
          />
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Give your planet a name..."
            className="w-full bg-white/10 rounded-xl p-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
          <textarea
            value={description}
            onChange={(e) => setDescription(e.target.value)}
            placeholder="Optional description..."
            className="w-full h-24 bg-white/10 rounded-xl p-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
          <div className="flex justify-center gap-4">
            <GlowButton onClick={handleSave} disabled={loading}>
              {loading ? 'Saving...' : 'Save Planet'}
            </GlowButton>
            <button
              onClick={handleDiscard}
              className="px-6 py-3 rounded-full bg-white/10 text-gray-300 hover:bg-white/20 transition"
            >
              Discard
            </button>
          </div>
      ...
      {!connected && (
        <p className="text-sm text-gray-400">Connect wallet to generate planets.</p>
      )}

      {/* Payment Modal (one‑time premium) */}
      {showPayment && (
      ...
      {loading && (
        <div className="flex justify-center">
          <motion.div className="w-12 h-12 border-4 border-purple-500 border-t-transparent rounded-full"
