...
export default function CreatePlanet() {
  ...
  const handleGenerate = () => {
    ...
    if (!prompt.trim()) { alert('Please describe your planet first.'); return }
  ...
  const handleSave = async () => {
    if (!name.trim()) { alert('Please enter a planet name.'); return }
    setLoading(true)
    try {
    ...
    } catch (err) { alert('Failed to save planet.') }
    setLoading(false)
  }
  ...
  return (
    <div className="relative min-h-screen">
      <SpaceBackground />
      <Navbar />
      <main className="pt-24 px-8 max-w-2xl mx-auto text-center space-y-10">
        <h2 className="text-4xl font-bold text-gradient">Create a New Planet</h2>
        ...
        {!preview ? (
          <>
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Describe your planet..."
              className="w-full h-32 bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500" />
          ...
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="space-y-6">
            <img src={preview.image_url} className="mx-auto w-64 h-64 rounded-full shadow-[0_0_50px_rgba(168,85,247,0.5)]" />
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Give your planet a name..."
              className="w-full bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500" />
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Optional description..."
              className="w-full h-24 bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-3 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500" />
            <div className="flex justify-center gap-4">
              <GlowButton onClick={handleSave} disabled={loading}>{loading ? 'Saving...' : 'Save Planet'}</GlowButton>
              <button onClick={handleDiscard} className="px-6 py-3 rounded-full bg-white/10 text-gray-300 hover:bg-white/20 transition">Discard</button>
            </div>
          </motion.div>
        )}
        {showPayment && (
        ...
        {loading && <div className="flex justify-center"><motion.div className="w-12 h-12 border-4 border-purple-500 border-t-transparent rounded-full" animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }} /></div>}
      </main>
    </div>
  )
}
