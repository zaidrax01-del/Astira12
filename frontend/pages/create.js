...
export default function CreatePlanet() {
  ...
  // Fetch user status on mount / when account changes
  useEffect(() => {
    if (account) {
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      setPreview({
        ...
        // default placeholders for the right panel
  ...
  // Default planet info for right panel
  const defaultPlanet = {
    image: '/planet-texture.jpg',   // placeholder image in public/
    name: 'Asteria Prime',
    type: 'Oceanic Planet',
    rarity: 'Epic',
    habitability: '78%',
          ...
          >
            ...
            >
              ...
              <p className="text-gray-300 mb-4">
                One‑time payment of ${PREMIUM_PRICE_USD} USD. Unlimited planets forever.
              </p>
              <div className="flex justify-center gap-4 mt-6">
                <button
                  onClick={() => handleUnlockPremium('SOL')}
                  style={{
                    padding: '14px 28px',
                    borderRadius: '999px',
