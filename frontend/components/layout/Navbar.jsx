  ...
  return (
    <nav className="fixed top-0 inset-x-0 z-50 px-6 py-4 flex items-center justify-between bg-black/20 backdrop-blur-xl border-b border-white/5">
      ...
      <div className="hidden md:flex gap-8 text-sm font-medium">
        {links.map(link => (
          <Link key={link.name} href={link.path} className="text-gray-300 hover:text-purple-400 transition">
            {link.name}
          </Link>
        ))}
      </div>
      <div className="flex items-center gap-4">
        ...
        {connected ? (
          <button onClick={disconnect} className="px-4 py-2 rounded-full bg-red-500/20 text-red-300 text-sm">
            Disconnect
          </button>
        ) : (
